...
NUM_OBJECTS = 8
PLAYER, LOCKED_ROOM, UNLOCKED_ROOM, KEY, BOUNDARY, FIRE, WALL, PERSON = range(NUM_OBJECTS)
...
def get_wall_locs(obs):
    locs = []
    for lit in obs:
        if lit.predicate.name.lower() == 'wallat':
            yield loc_str_to_loc(lit.variables[1])

def get_fire_locs(obs):
    locs = []
    for lit in obs:
        if lit.predicate.name.lower() == 'fireat':
            yield loc_str_to_loc(lit.variables[1])

def get_person_locs(obs):
    locs = []
    for lit in obs:
        if lit.predicate.name.lower() == 'personat':
            yield loc_str_to_loc(lit.variables[1])
...
def build_layout(obs):
    ...
    person_locs = get_person_locs(obs)
    for person_loc in person_locs:
        layout[person_loc[0], person_loc[1], PERSON] = 1

    wall_locs = get_wall_locs(obs)
    for wall_loc in wall_locs:
        layout[wall_loc[0], wall_loc[1], WALL] = 1
    
    fire_locs = get_fire_locs(obs)
    for fire_loc in fire_locs:
        layout[fire_loc[0], fire_loc[1], FIRE] = 1
...
def get_token_images(obs_cell):
    ...
    if obs_cell[FIRE]:
        yield TOKEN_IMAGES[FIRE]
    if obs_cell[WALL]:
        yield TOKEN_IMAGES[WALL]
    if obs_cell[PERSON]:
        yield TOKEN_IMAGES[PERSON]
    return
