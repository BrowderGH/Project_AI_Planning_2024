...
def run_demo(env, policy, max_num_steps=10, render=True,
             video_path=None, fps=3, verbose=False, seed=None,
             check_reward=False):

    images = []
    print(render)
    print("QQQQQQQQQQQQQTest")
    if seed is not None:
        env.seed(seed)
    ...
    if render:
       images.append(env.render())
       print(render)
       print("ZZZZZTTTTTTTTTTesting")
       LNG = len(images)
       print("The number of images is:", LNG)
       imageio.mimwrite(video_path, images)
       #imageio.mimwrite(video_path, images, bigtiff = True)
       #imageio.mimwrite(video_path, images, format="PNG-PIL")
