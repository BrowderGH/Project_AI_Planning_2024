...
#sys.stdout = io.open(f"a__Output_Run_Lookahead/log", 'w')
...
for i in range(1, 11):
    ...
    #MB:  index number refers to the pddl problem file 
    ...
    imageio.imsave(f"a__Output_Run_Lookahead/Image1.png", img)
...
###########################################################################################
    ...
    def Observe_Current_State(obs_RW, obs_PW, Planned_Destinations, Planned_Actions, index, loop_check):
  
        global check, appended_removals, Journey
    ...
    def Run_Lookahead (env_Real_World, env_Perceived_World, obs_RW, obs_PW):

        global start_time, end_time, Tot_Actions_in_Plans, Journey

        Journey = "Start"
        View_WorldCheck = "Empty"
        ...
        while True:
            ...
            """
            """
            if Journey == "Start" or View_WorldCheck == "Failure":
            ...
            if Journey != "Start":
                obs_PW, Observe_Current_State_ = Observe_Current_State(obs_RW, obs_PW, Planned_Destinations, Planned_Actions, index, loop_check)
            ...
            View_WorldCheck = "Proceed"
                
            Journey = "In_Motion"
    
            #since this is Run Lookahead, we will do this loop once:
            for act in plan_PW:
                print(" ")
                print(" ")
                ...
                count += 1
                Action_Count += 1
                imageio.imsave(f"a__Output_Run_Lookahead/Image{Action_Count}.png", img)
                break

            #check where we are in plan:
            remaining_actions = 0
            for act in plan_PW:
    ...
    obs_PW = "null"
    Run_Lookahead(env_Real_World, env_Perceived_World, obs_RW, obs_PW)
...
# I also do all these stats again elsewhere in different program
...
eval("DF_nums").to_csv(f"a__Output_Run_Lookahead/{Group_Name}_Main.csv", header=True)
...
ax.text( .5, .95, "A* Hff for 16% Fireballs for Run Lookahead", transform=ax.transAxes, weight = "bold", ha='center', fontsize=14)
...
plt.savefig(f"a__Output_Run_Lookahead/{Group_Name}.png")
plt.show()
