...
F1 = A1[["Pl Time", "Fireballs", "Search"]]
F2 = A2[["Pl Time", "Fireballs", "Search"]]
F3 = A3[["Pl Time", "Fireballs", "Search"]]
F4 = A4[["Pl Time", "Fireballs", "Search"]]
F5 = B1[["Pl Time", "Fireballs", "Search"]]
F6 = B2[["Pl Time", "Fireballs", "Search"]]
F7 = B3[["Pl Time", "Fireballs", "Search"]]
F8 = B4[["Pl Time", "Fireballs", "Search"]]
F9 = C1[["Pl Time", "Fireballs", "Search"]]
F10 = C2[["Pl Time", "Fireballs", "Search"]]
F11 = C3[["Pl Time", "Fireballs", "Search"]]
F12 = C4[["Pl Time", "Fireballs", "Search"]]

Plan_T = pd.DataFrame(pd.concat([F1, F2, F3, F4, F5, F6, F7, F8, F9, F10, F11, F12], axis = 0))


Plan_T.rename(columns={"Pl Time":"Planning_Time"}, inplace=True)
Plan_T.info()
...
model = ols("Planning_Time ~ C(Fireballs) + C(Search) + C(Fireballs):C(Search)", 
            data=Plan_T).fit() 
output_A = sm.stats.anova_lm(model, type=2) 
#print(Plan_T.loc[0:20, :])
...
eval("output_A").to_csv(f"a__Output_ANOVA/Planning_Time_Against_Search_Method_ANOVA.csv", header=True)
...
ax.text( .5, .95, "ANOVA for Planning Time by Fireballs", transform=ax.transAxes, weight = "bold", ha='center', fontsize=14)
...
plt.savefig(f"a__Output_ANOVA/Planning_Time_Against_Search_Method_Picture_A.png")
plt.close()


#  Do two way ANOVA for Planning_Time, Fireballs, and Heuristic
########################################################################################
########################################################################################


F1 = A1[["Pl Time", "Fireballs", "Heuristic"]]
F2 = A2[["Pl Time", "Fireballs", "Heuristic"]]
F3 = A3[["Pl Time", "Fireballs", "Heuristic"]]
F4 = A4[["Pl Time", "Fireballs", "Heuristic"]]
F5 = B1[["Pl Time", "Fireballs", "Heuristic"]]
F6 = B2[["Pl Time", "Fireballs", "Heuristic"]]
F7 = B3[["Pl Time", "Fireballs", "Heuristic"]]
F8 = B4[["Pl Time", "Fireballs", "Heuristic"]]
F9 = C1[["Pl Time", "Fireballs", "Heuristic"]]
F10 = C2[["Pl Time", "Fireballs", "Heuristic"]]
F11 = C3[["Pl Time", "Fireballs", "Heuristic"]]
F12 = C4[["Pl Time", "Fireballs", "Heuristic"]]
...
model = ols("Planning_Time ~ C(Fireballs) + C(Heuristic) + C(Fireballs):C(Heuristic)", 
            data=Plan_T).fit() 
output_A = sm.stats.anova_lm(model, type=2) 
#print(Plan_T.loc[0:20, :])
...
eval("output_A").to_csv(f"a__Output_ANOVA/Planning_Time_Against_Heuristic_ANOVA.csv", header=True)
...
plt.savefig(f"a__Output_ANOVA/Planning_Time_Against_Heuristic_Picture_A.png")
plt.close()
