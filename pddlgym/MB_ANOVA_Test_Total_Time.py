...
#  Do two way ANOVA for Total Time, Fireballs, and Search Method
#########################################################################################
#########################################################################################

F1 = A1[["Tot Time", "Fireballs", "Search"]]
F2 = A2[["Tot Time", "Fireballs", "Search"]]
F3 = A3[["Tot Time", "Fireballs", "Search"]]
F4 = A4[["Tot Time", "Fireballs", "Search"]]
F5 = B1[["Tot Time", "Fireballs", "Search"]]
F6 = B2[["Tot Time", "Fireballs", "Search"]]
F7 = B3[["Tot Time", "Fireballs", "Search"]]
F8 = B4[["Tot Time", "Fireballs", "Search"]]
F9 = C1[["Tot Time", "Fireballs", "Search"]]
F10 = C2[["Tot Time", "Fireballs", "Search"]]
F11 = C3[["Tot Time", "Fireballs", "Search"]]
F12 = C4[["Tot Time", "Fireballs", "Search"]]

Tot_T = pd.DataFrame(pd.concat([F1, F2, F3, F4, F5, F6, F7, F8, F9, F10, F11, F12], axis = 0))


Tot_T.rename(columns={"Tot Time":"Total_Time"}, inplace=True)
Tot_T.info()
...
model = ols("Total_Time ~ C(Fireballs) + C(Search) + C(Fireballs):C(Search)", 
            data=Tot_T).fit() 
output_A = sm.stats.anova_lm(model, type=2) 
#print(Tot_T.loc[0:20, :])
...
eval("output_A").to_csv(f"a__Output_ANOVA/Total_Time_Against_Search_Method_ANOVA.csv", header=True)
...
ax.text( .5, .95, "ANOVA for Total Time by Fireballs", transform=ax.transAxes, weight = "bold", ha='center', fontsize=14)
...
plt.savefig(f"a__Output_ANOVA/Total_Time_Against_Search_Method_Picture_A.png")
plt.close()


#  Do two way ANOVA for Total_Time, Fireballs, and Heuristic
########################################################################################
########################################################################################


F1 = A1[["Tot Time", "Fireballs", "Heuristic"]]
F2 = A2[["Tot Time", "Fireballs", "Heuristic"]]
F3 = A3[["Tot Time", "Fireballs", "Heuristic"]]
F4 = A4[["Tot Time", "Fireballs", "Heuristic"]]
F5 = B1[["Tot Time", "Fireballs", "Heuristic"]]
F6 = B2[["Tot Time", "Fireballs", "Heuristic"]]
F7 = B3[["Tot Time", "Fireballs", "Heuristic"]]
F8 = B4[["Tot Time", "Fireballs", "Heuristic"]]
F9 = C1[["Tot Time", "Fireballs", "Heuristic"]]
F10 = C2[["Tot Time", "Fireballs", "Heuristic"]]
F11 = C3[["Tot Time", "Fireballs", "Heuristic"]]
F12 = C4[["Tot Time", "Fireballs", "Heuristic"]]
...
model = ols("Total_Time ~ C(Fireballs) + C(Heuristic) + C(Fireballs):C(Heuristic)", 
            data=Tot_T).fit() 
output_A = sm.stats.anova_lm(model, type=2) 
#print(Tot_T.loc[0:20, :])
...
eval("output_A").to_csv(f"a__Output_ANOVA/Total_Time_Against_Heuristic_ANOVA.csv", header=True)
...
plt.savefig(f"a__Output_ANOVA/Total_Time_Against_Heuristic_Picture_A.png")
plt.close()
